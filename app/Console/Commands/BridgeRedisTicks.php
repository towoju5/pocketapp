...
/**
 * Bridges Redis Streams to the frontend: StreamBrokeretTicks
 * (app/Console/Commands/StreamBrokeretTicks.php, `php artisan
 * ticks:stream-brokeret`) connects to Brokeret's feed directly and writes
 * ticks straight into Redis (ticks:{symbol} streams, see PriceFeedService's
 * docblock for the schema) via PriceFeedService, with no knowledge of this
 * app's WebSocket layer. This process only ever tails ticks:* streams for
 * newly-appended entries and rebroadcasts them as AssetPriceBatchUpdated
 * over Reverb, so chart.js's ChartManager._initBroadcast and every other
 * Echo listener need no changes if the writer ever changes again (the
 * iqcent-based collector this replaced used to emit the same event via
 * TickerController::collectBatch, which is why this bridge has always been
 * writer-agnostic).
 *
