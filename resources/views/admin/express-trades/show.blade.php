    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <x-glass-card title="Manual Settlement">
            @if ($trade->trade_status === 'open')
                ...
                <form method="POST" action="{{ route('admin.express-trades.force-win', $trade) }}" class="space-y-3" onsubmit="return confirm('Force this trade to WIN? The payout will be credited immediately.');">
                    @csrf
                ...
                <form method="POST" action="{{ route('admin.express-trades.force-lose', $trade) }}" class="mt-3 space-y-3" onsubmit="return confirm('Force this trade to LOSE?');">
                    @csrf
                    <textarea name="notes" rows="2" placeholder="Reason (required, kept on the trade's audit trail)" class="brand-input-dark" required></textarea>
                ...
                <form method="POST" action="{{ route('admin.express-trades.void', $trade) }}" class="mt-3 space-y-3" onsubmit="return confirm('Void this trade and refund the stake?');">
                    @csrf
                    <textarea name="notes" rows="2" placeholder="Reason (required, kept on the trade's audit trail)" class="brand-input-dark" required></textarea>
