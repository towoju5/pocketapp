<div id="sideMenu" class="bg-[#1c1f26] min-h-screen px-0 py-1 lg:w-24">
    <!-- Menu Items -->
    <button @if(Route::is('trade.index')) bg-slate-400 @endif data-href="{{ route('trade.index') }}" class="menu-item py-3 hover:bg-slate-400 w-full text-gray-400 hover:text-white" data-tooltip="Analytics">
        <i class="fa-solid fa-chart-line"></i>
        <span class="hidden_text hidden lg:block">Trading</span>
    </button>

    <button @if(Route::is('trade.index')) bg-slate-400 @endif data-href="{{ route('deposit.index') }}" class="menu-item py-3 w-full text-gray-400 hover:text-white"
        data-tooltip="Finance">
        <i class="fa-solid fa-dollar-sign"></i>
        <span class="hidden_text hidden lg:block">Finance</span>
    </button>

    <button @if(Route::is('trade.index')) bg-slate-400 @endif data-href="{{ route('profile.edit') }}" class="menu-item py-3 w-full text-gray-400 hover:text-white"
        data-tooltip="Profile">
        <i class="fa-solid fa-user"></i>
        <span class="hidden_text hidden lg:block">Profile</span>
    </button>

    <button @if(Route::is('trade.index')) bg-slate-400 @endif data-href="#" class="menu-item py-3 w-full text-gray-400 hover:text-white relative" data-tooltip="Cart">
        <i class="fa-solid fa-cart-shopping"></i>
        <span class="hidden_text hidden lg:block">Market</span>
    </button>

    <button data-href="#" class="menu-item py-3 w-full text-gray-400 hover:text-white relative"
    ...
    <button data-href="#" class="menu-item py-3 w-full text-gray-400 hover:text-white relative" data-tooltip="Chat">
        <i class="fa-regular fa-comment"></i>
        <span class="absolute top-2 right-2 bg-[#0c69a9] text-xs rounded-md text-white px-2 py-1">6</span>
        <span class="hidden_text hidden lg:block">Chat</span>
    </button>

    <button data-href="#" class="menu-item py-3 w-full text-gray-400 hover:text-white" data-tooltip="Help">
