<nav class="flex justify-between items-center p-4 bg-gray-900/95 backdrop-blur-sm" role="navigation"
  aria-label="Main navigation">
  <div class="flex items-center space-x-3">
    <!-- Profile Icon -->
    <button class="relative" aria-label="Profile status">
      <svg class="w-12 h-12 bg-gray-700 rounded-full border-2 border-green-500" viewBox="0 0 25.916 25.916"
        xmlns="http://www.w3.org/2000/svg" fill="#000">
        <g fill="#c4dede">
          <path
            d="M7.938 8.13c.09.414.228.682.389.849.383 2.666 2.776 4.938 4.698 4.843 2.445-.12 4.178-2.755 4.567-4.843.161-.166.316-.521.409-.938.104-.479.216-1.201-.072-1.583-.017-.02-.127-.121-.146-.138.275-.992.879-2.762-.625-4.353-.815-.862-1.947-1.295-2.97-1.637-3.02-1.009-5.152.406-6.136 2.759C7.981 3.256 7.522 4.313 8.078 6.32c-.054.036-.103.082-.144.138-.289.381-.101 1.193.004 1.672z" />
          <path
            d="M23.557 22.792c-.084-1.835-.188-4.743-1.791-7.122 0 0-.457-.623-1.541-1.037 0 0-2.354-.717-3.438-1.492l-.495.339.055 3.218-2.972 7.934a.461.461 0 0 1-.832 0l-2.971-7.934s.055-3.208.054-3.218c.007.027-.496-.339-.496-.339-1.082.775-3.437 1.492-3.437 1.492-1.084.414-1.541 1.037-1.541 1.037-1.602 2.379-1.708 5.287-1.792 7.122-.058 1.268.208 1.741.542 1.876 4.146 1.664 15.965 1.664 20.112 0 .334-.135.598-.609.541-1.876z" />
          <path
            d="M13.065 14.847l-.134.003a4.18 4.18 0 0 1-1.296-.232l1.178 1.803-1.057 1.02 1.088 6.607a.116.116 0 0 0 .232 0l1.088-6.607-1.058-1.02 1.161-1.776a4.02 4.02 0 0 1-1.323.205z" />
        </g>
      </svg>

      <svg class="w-3 h-3 bg-green-500 rounded-full absolute top-0 right-0" viewBox="0 0 24 24" fill="none"
        xmlns="http://www.w3.org/2000/svg">
        <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
        <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
        <g id="SVGRepo_iconCarrier">
          <path fill-rule="evenodd" clip-rule="evenodd"
            d="M13.7137 3.90806C12.9363 2.61642 11.0637 2.61642 10.2864 3.90806L8.39925 7.04391L4.83371 7.86969C3.36509 8.20983 2.78642 9.99077 3.77464 11.1292L6.17384 13.893L5.85739 17.5392C5.72705 19.0411 7.24201 20.1418 8.63008 19.5537L12 18.126L15.37 19.5537C16.7581 20.1418 18.273 19.0411 18.1427 17.5392L17.8263 13.893L20.2254 11.1292C21.2137 9.99077 20.635 8.20983 19.1664 7.86969L15.6008 7.0439L13.7137 3.90806Z"
            fill="#ffffff"></path>
        </g>
      </svg>
    </button>

    <!-- Percentage -->
    <div class="hidden bg-gray-800 px-2 py-1 rounded-full border-2 border-green-500 text-xs hover:bg-gray-700 transition-colors cursor-pointer flex items-center justify-center w-12 h-12">
      25%
    </div>
  </div>

  <div class="flex items-center gap-2 relative">
    <!-- Gift Icon -->
    <button class="hidden bg-gray-800 p-2 rounded-md hover:bg-gray-700 transition-colors" aria-label="Gift options">
      <svg class="w-6 h-6 icon" viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg" fill="#000">
        <g id="SVGRepo_iconCarrier">
          <path
            d="M859.9 135.1V821H174V135.1h685.9z m-10.2 675.8V534.3h-212c0 0.2 0.1 0.4 0.1 0.5 9.1 19.2 7.3 41.3-4.9 59-11.6 16.9-31.1 27.1-53.4 28.8V810h-10.1V622.8c-0.9-0.1-1.8 0-2.8-0.1-14.4-1-26.1-6.8-34.7-17.2-5.7-6.8-9.4-15-11.9-23.3-2.4 8.3-6.2 16.5-11.9 23.3-8.6 10.4-20.2 16.2-34.6 17.2-3 0.2-6 0.3-8.9 0.2v188h385.1z"
            fill="#41D6D2"></path>
          <path
            d="M849.7 534.3v276.6H569.4v-0.9h10.1V622.6c22.3-1.6 41.7-11.8 53.4-28.8 12.1-17.7 14-39.8 4.9-59 0-0.1-0.1-0.3-0.1-0.5h212z"
            fill="#A5EFEB"></path>
          <path d="M849.7 421.1v103.1H630.9c-10.4-12.5-35-32.7-89.7-42.3 69.7-12.4 90.7-41.9 95.5-50.6h211.6v-10.1h1.4z"
            fill="#D05C86"></path>
          <path d="M849.7 145.3v275.9H640.8c5.4-17.3 2.6-35.9-7.9-51.3-11.6-16.9-31.1-27.1-53.4-28.8V145.3h270.2z"
            fill="#A5EFEB"></path>
        </g>
      </svg>
    </button>


    <div class="flex relative gap-2">
      <!-- QT Real button -->
      <button
        class="bg-gray-800 px-3 py-2 rounded-md flex items-center space-x-2 hover:bg-gray-700 transition-colors dropdown-toggle"
        aria-expanded="false" aria-controls="dropModal" id="accountDropdown"
        data-modal-target="q_dropModal" data-modal-toggle="q_dropModal">
        <div>
          <span class="text-xs text-gray-400 account-type">QT Real NGN</span>
          <span class="text-lg font-medium account-balance">0</span>
        </div>
        <span class="bg-gray-700 p-1 rounded-md">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
          </svg>
        </span>
      </button>

      <!-- Wallet button -->
      <button class="bg-green-600 p-2 rounded-md hover:bg-green-700 transition-colors" aria-label="Open wallet"
        id="walletButton" type="button" data-modal-target="walletModal" data-modal-toggle="walletModal">
        <svg class="w-6 h-6 h-btn--deposit" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path
            d="M18 8V7.2C18 6.0799 18 5.51984 17.782 5.09202C17.5903 4.71569 17.2843 4.40973 16.908 4.21799C16.4802 4 15.9201 4 14.8 4H6.2C5.07989 4 4.51984 4 4.09202 4.21799C3.71569 4.40973 3.40973 4.71569 3.21799 5.09202C3 5.51984 3 6.0799 3 7.2V8M21 12H19C17.8954 12 17 12.8954 17 14C17 15.1046 17.8954 16 19 16H21M3 8V16.8C3 17.9201 3 18.4802 3.21799 18.908C3.40973 19.2843 3.71569 19.5903 4.09202 19.782C4.51984 20 5.07989 20 6.2 20H17.8C18.9201 20 19.4802 20 19.908 19.782C20.2843 19.5903 20.5903 19.2843 20.782 18.908C21 18.4802 21 17.9201 21 16.8V11.2C21 10.0799 21 9.51984 20.782 9.09202C20.5903 8.71569 20.2843 8.40973 19.908 8.21799C19.4802 8 18.9201 8 17.8 8H3Z"
            stroke="#ffffff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
        </svg>
      </button>

    </div>
  </div>
</nav>

  <!-- Wallet Modal -->
  <div id="walletModal" tabindex="-1" aria-hidden="true"
    class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative w-80 transform bg-gray-800 rounded-xl shadow-xl">
      @include('components.wallet-modal')
    </div>
  </div>

  <!-- QT Real Modal -->
  <div id="q_dropModal" tabindex="-1" aria-hidden="true"
    class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div style="background: #262c41; border: 1px solid #343e52"
      class="relative p-3 transform w-80 rounded-lg shadow-xl">
      <div class="p-2 rounded-lg w-full" style="background: #1f2536">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-2">
            <div>
              <svg class="svg-icon qt-real" xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                viewBox="0 0 24 24" fill="none">
                <path fill="currentColor"
                  d="M7.832 1.858C8.822 1.308 10.12 1 11.5 1s2.678.309 3.668.858C16.123 2.388 17 3.282 17 4.5v16.11c0 1.207-.901 2.069-1.842 2.57-.984.525-2.278.82-3.658.82s-2.673-.295-3.658-.82C6.901 22.68 6 21.818 6 20.61V4.5c0-1.218.877-2.111 1.832-2.642ZM8 7.231V8.5c0 .333.171.67.77.98c.622.32 1.572.52 2.73.52s2.108-.2 2.73-.52c.599-.31.77-.647.77-.98V7.231c-.966.494-2.197.769-3.5.769S8.966 7.725 8 7.231ZM15 4.5c0 .162-.13.52-.804.894-.64.355-1.59.606-2.696.606-1.105 0-2.057-.251-2.696-.606C8.13 5.019 8 4.662 8 4.5c0-.162.13-.52.804-.894C9.444 3.251 10.394 3 11.5 3c1.105 0 2.057.251 2.696.606.674.375.804.732.804.894Zm0 6.83c-.982.466-2.222.67-3.5.67s-2.518-.204-3.5-.67v1.17c0 .333.171.67.77.98c.622.32 1.572.52 2.73.52s2.108-.2 2.73-.52c.599-.31.77-.647.77-.98v-1.17Zm0 4c-.982.466-2.222.67-3.5.67s-2.518-.204-3.5-.67v1.17c0 .333.171.67.77.98c.622.32 1.572.52 2.73.52s2.108-.2 2.73-.52c.599-.31.77-.647.77-.98v-1.17Zm0 4c-.982.466-2.222.67-3.5.67s-2.518-.204-3.5-.67v1.28c0 .108.099.441.783.806.64.341 1.597.584 2.717.584s2.076-.243 2.717-.584c.684-.365.783-.698.783-.805V19.33Z" />
                <path fill="currentColor"
                  d="M16.584 21.951c.416.049.4.049.916.049 1.38 0 2.674-.295 3.658-.82.941-.501 1.842-1.363 1.842-2.57V14.5c0-1.218-.877-2.111-1.832-2.642-.99-.55-2.288-.858-3.668-.858-.515 0-.02-.082-.5 0v2c.45-.104-.044 0 .5 0 1.105 0 2.057.251 2.696.606.674.374.804.732.804.894 0 .162-.13.52-.804.894-.64.355-1.59.606-2.696.606-.544 0-.05.104-.5 0v2h.5c1.303 0 2.534-.275 3.5-.769v1.38c0 .107-.099.44-.783.805-.64.341-1.597.584-2.717.584H17l-.416 1.951ZM6 6.014A9.163 9.163 0 0 0 5.5 6c-1.38 0-2.679.309-3.668.858C.877 7.388 0 8.282 0 9.5c0 .104.006.206.019.306A1.005 1.005 0 0 0 0 10v7.61c0 1.207.901 2.069 1.842 2.57.985.525 2.278.82 3.658.82.168 0 .335-.004.5-.013v-2.003c-.163.01-.33.016-.5.016-1.12 0-2.077-.243-2.717-.584C2.099 18.05 2 17.718 2 17.61v-1.508c.966.573 2.193.897 3.5.897.168 0 .335-.005.5-.016V14.98a5.83 5.83 0 0 1-.5.021c-1.08 0-2.005-.293-2.631-.712C2.236 13.864 2 13.388 2 13v-.769c.966.494 2.197.769 3.5.769.168 0 .335-.005.5-.014v-2.003a7.43 7.43 0 0 1-.5.017c-1.105 0-2.057-.251-2.696-.606C2.13 10.02 2 9.662 2 9.5c0-.162.13-.52.804-.894C3.444 8.251 4.394 8 5.5 8c.17 0 .337.006.5.017V6.014Z" />
              </svg>
            </div>
            <div class="text-white text-sm font-light">
              <p>Quick Trading Real</p>
              <p>#0</p>
            </div>
          </div>
          <p class="px-3 py-1 text-sm rounded border-2 border-dotted border-blue-700">
            NGN
          </p>
        </div>
        <div class="mt-3 flex items-center gap-2">
          <div class="w-3/5">
            <button
              class="font-light text-sm py-1 w-full flex flex-row items-center justify-center gap-2 rounded-md"
              style="background-color: #074f40; border: 1px solid #047838">
              <svg width="17" height="16" viewBox="0 0 17 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path
                  d="M14.1 4.00005H13.3V3.20005C13.3 2.56353 13.0471 1.05291 12.5971 1.50299C12.147 1.05291 11.5365 0.800049 10.9 0.800049H2.9C2.26348 0.800049 1.65303 1.05291 1.20294 1.50299C0.752856 1.95308 0.5 2.56353 0.5 3.20005V12.8C0.5 13.4366 0.752856 14.047 1.20294 14.4971C1.65303 14.9472 2.26348 15.2 2.9 15.2H14.1C14.7365 15.2 15.347 14.9472 15.7971 14.4971C16.2471 14.047 16.5 13.4366 16.5 12.8V6.40005C16.5 5.76353 16.2471 5.15308 15.7971 4.70299C15.347 4.25291 14.7365 4.00005 14.1 4.00005ZM2.9 2.40005H10.9C11.1122 2.40005 11.3157 2.48433 11.4657 2.63436C11.6157 2.78439 11.7 2.98788 11.7 3.20005V4.00005H2.9C2.68783 4.00005 2.48434 3.91576 2.33431 3.76573C2.18429 3.6157 2.1 3.41222 2.1 3.20005C2.1 2.98788 2.18429 2.78439 2.33431 2.63436C2.48434 2.48433 2.68783 2.40005 2.9 2.40005ZM14.9 10.4H14.1C13.8878 10.4 13.6843 10.3158 13.5343 10.1657C13.3843 10.0157 13.3 9.81222 13.3 9.60005C13.3 9.38788 13.3843 9.18439 13.5343 9.03436C13.6843 8.88433 13.8878 8.80005 14.1 8.80005H14.9V10.4ZM14.9 7.20005H14.1C13.4635 7.20005 12.853 7.4529 12.4029 7.90299C11.9529 8.35308 11.7 8.96353 11.7 9.60005C11.7 10.2366 11.9529 10.847 12.4029 11.2971C12.853 11.7472 13.4635 12 14.1 12H14.9V12.8C14.9 13.0122 14.8157 13.2157 14.6657 13.3657C14.5157 13.5158 14.3122 13.6 14.1 13.6H2.9C2.68783 13.6 2.48434 13.5158 2.33431 13.3657C2.18429 13.2157 2.1 13.0122 2.1 12.8V5.46405C2.35701 5.55446 2.62755 5.60045 2.9 5.60005H14.1C14.3122 5.60005 14.5157 5.68433 14.6657 5.83436C14.8157 5.98439 14.9 6.18788 14.9 6.40005V7.20005Z"
                  fill="currentColor"></path>
              </svg>
              Top up
            </button>
          </div>
          <div class="w-2/5 flex items-center gap-2">
            <button class="font-light text-sm py-1 w-full flex items-center justify-center rounded-md"
              style="background: #18374a; border: 1px solid #009af9">
              <svg width="18" height="16" viewBox="0 0 18 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path
                  d="M8.99995 8C8.52528 8 8.06126 8.14076 7.66658 8.40447C7.2719 8.66819 6.96429 9.04302 6.78264 9.48156C6.60099 9.9201 6.55346 10.4027 6.64607 10.8682C6.73867 11.3338 6.96725 11.7614 7.30289 12.0971C7.63854 12.4327 8.06618 12.6613 8.53173 12.7539C8.99729 12.8465 9.47985 12.799 9.91839 12.6173C10.3569 12.4357 10.7318 12.128 10.9955 11.7334C11.2592 11.3387 11.4 10.8747 11.4 10.4C11.4 9.76348 11.1471 9.15303 10.697 8.70294C10.2469 8.25286 9.63647 8 8.99995 8ZM8.99995 11.2C8.84173 11.2 8.68705 11.1531 8.5555 11.0652C8.42394 10.9773 8.3214 10.8523 8.26085 10.7061C8.2003 10.56 8.18446 10.3991 8.21532 10.2439C8.24619 10.0887 8.32238 9.9462 8.43427 9.83432C8.54615 9.72243 8.68869 9.64624 8.84388 9.61537C8.99906 9.5845 9.15992 9.60035 9.3061 9.6609C9.45228 9.72145 9.57722 9.82398 9.66513 9.95554C9.75303 10.0871 9.79995 10.2418 9.79995 10.4C9.79995 10.6122 9.71567 10.8157 9.56564 10.9657C9.41561 11.1157 9.21212 11.2 8.99995 11.2ZM8.43195 6.168C8.50803 6.24083 8.59775 6.29792 8.69595 6.336C8.79171 6.37832 8.89526 6.40019 8.99995 6.40019C9.10465 6.40019 9.20819 6.37832 9.30395 6.336C9.40215 6.29792 9.49187 6.24083 9.56795 6.168L11.4 4.368C11.5548 4.21311 11.6419 4.00304 11.6419 3.784C11.6419 3.56496 11.5548 3.35489 11.4 3.2C11.2451 3.04511 11.035 2.9581 10.816 2.9581C10.5969 2.9581 10.3868 3.04511 10.232 3.2L9.79995 3.672V0.8C9.79995 0.587827 9.71567 0.384344 9.56564 0.234315C9.41561 0.0842854 9.21212 0 8.99995 0C8.78778 0 8.5843 0.0842854 8.43427 0.234315C8.28424 0.384344 8.19995 0.587827 8.19995 0.8V3.672L7.76795 3.2C7.61307 3.04511 7.40299 2.9581 7.18395 2.9581C6.96491 2.9581 6.75484 3.04511 6.59995 3.2C6.44507 3.35489 6.35805 3.56496 6.35805 3.784C6.35805 4.00304 6.44507 4.21311 6.59995 4.368L8.43195 6.168ZM14.6 10.4C14.6 10.2418 14.553 10.0871 14.4651 9.95554C14.3772 9.82398 14.2523 9.72145 14.1061 9.6609C13.9599 9.60035 13.7991 9.5845 13.6439 9.61537C13.4887 9.64624 13.3461 9.72243 13.2343 9.83432C13.1224 9.9462 13.0462 10.0887 13.0153 10.2439C12.9845 10.3991 13.0003 10.56 13.0608 10.7061C13.1214 10.8523 13.2239 10.9773 13.3555 11.0652C13.4871 11.1531 13.6417 11.2 13.8 11.2C14.0121 11.2 14.2156 11.1157 14.3656 10.9657C14.5157 10.8157 14.6 10.6122 14.6 10.4ZM15.4 4.8H13C12.7878 4.8 12.5843 4.88429 12.4343 5.03431C12.2842 5.18434 12.2 5.38783 12.2 5.6C12.2 5.81217 12.2842 6.01566 12.4343 6.16569C12.5843 6.31571 12.7878 6.4 13 6.4H15.4C15.6121 6.4 15.8156 6.48429 15.9656 6.63432C16.1157 6.78434 16.2 6.98783 16.2 7.2V13.6C16.2 13.8122 16.1157 14.0157 15.9656 14.1657C15.8156 14.3157 15.6121 14.4 15.4 14.4H2.59995C2.38778 14.4 2.18429 14.3157 2.03427 14.1657C1.88424 14.0157 1.79995 13.8122 1.79995 13.6V7.2C1.79995 6.98783 1.88424 6.78434 2.03427 6.63432C2.18429 6.48429 2.38778 6.4 2.59995 6.4H4.99995C5.21212 6.4 5.41561 6.31571 5.56564 6.16569C5.71567 6.01566 5.79995 5.81217 5.79995 5.6C5.79995 5.38783 5.71567 5.18434 5.56564 5.03431C5.41561 4.88429 5.21212 4.8 4.99995 4.8H2.59995C1.96343 4.8 1.35298 5.05286 0.902895 5.50294C0.452808 5.95303 0.199951 6.56348 0.199951 7.2V13.6C0.199951 14.2365 0.452808 14.847 0.902895 15.2971C1.35298 15.7471 1.96343 16 2.59995 16H15.4C16.0365 16 16.6469 15.7471 17.097 15.2971C17.5471 14.847 17.8 14.2365 17.8 13.6V7.2C17.8 6.56348 17.5471 5.95303 17.097 5.50294C16.6469 5.05286 16.0365 4.8 15.4 4.8ZM3.39995 10.4C3.39995 10.5582 3.44687 10.7129 3.53478 10.8445C3.62268 10.976 3.74762 11.0786 3.8938 11.1391C4.03999 11.1997 4.20084 11.2155 4.35602 11.1846C4.51121 11.1538 4.65375 11.0776 4.76564 10.9657C4.87752 10.8538 4.95371 10.7113 4.98458 10.5561C5.01545 10.4009 4.99961 10.24 4.93905 10.0939C4.8785 9.94767 4.77597 9.82273 4.64441 9.73482C4.51285 9.64692 4.35818 9.6 4.19995 9.6C3.98778 9.6 3.7843 9.68429 3.63427 9.83432C3.48424 9.98434 3.39995 10.1878 3.39995 10.4Z"
                  fill="currentColor"></path>
              </svg>
            </button>
            <button class="font-light text-sm py-1 w-full flex items-center justify-center rounded-md"
              style="background: #18374a; border: 1px solid #009af9">
              <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                <circle cx="8" cy="8" r="7.1" stroke="currentColor" stroke-width="1.8"></circle>
                <path
                  d="M3.06408 6.3379C3.10216 6.2397 3.15925 6.14998 3.23208 6.0739L5.03208 4.2419C5.18697 4.08701 5.39704 4 5.61608 4C5.83512 4 6.04519 4.08701 6.20008 4.2419C6.35497 4.39679 6.44198 4.60686 6.44198 4.8259C6.44198 5.04494 6.35497 5.25501 6.20008 5.4099L5.72808 5.8419L7.93431 5.8419C8.14649 5.8419 8.34997 5.92619 8.5 6.07622C8.65003 6.22624 8.73431 6.42973 8.73431 6.6419C8.73431 6.85407 8.65003 7.05756 8.5 7.20759C8.34997 7.35761 8.14649 7.4419 7.93431 7.4419L5.72808 7.4419L6.20008 7.8739C6.35497 8.02879 6.44198 8.23886 6.44198 8.4579C6.44198 8.67694 6.35497 8.88701 6.20008 9.0419C6.04519 9.19679 5.83512 9.2838 5.61608 9.2838C5.39704 9.2838 5.18697 9.19679 5.03208 9.0419L3.23208 7.2099C3.15925 7.13382 3.10216 7.0441 3.06408 6.9459C3.02176 6.85014 2.99989 6.7466 2.99989 6.6419C2.99989 6.5372 3.02176 6.43366 3.06408 6.3379Z"
                  fill="currentColor"></path>
                <path
                  d="M12.6701 9.94591C12.632 10.0441 12.5749 10.1338 12.5021 10.2099L10.7021 12.0419C10.5472 12.1968 10.3371 12.2838 10.1181 12.2838C9.89901 12.2838 9.68894 12.1968 9.53405 12.0419C9.37916 11.887 9.29215 11.677 9.29215 11.4579C9.29215 11.2389 9.37916 11.0288 9.53405 10.8739L10.0061 10.4419L7.79982 10.4419C7.58764 10.4419 7.38416 10.3576 7.23413 10.2076C7.0841 10.0576 6.99982 9.85409 6.99982 9.64191C6.99982 9.42974 7.0841 9.22626 7.23413 9.07623C7.38416 8.9262 7.58764 8.84191 7.79982 8.84191L10.0061 8.84191L9.53405 8.40991C9.37916 8.25503 9.29215 8.04496 9.29215 7.82591C9.29215 7.60687 9.37916 7.3968 9.53405 7.24191C9.68894 7.08703 9.89901 7.00001 10.1181 7.00001C10.3371 7.00001 10.5472 7.08703 10.7021 7.24191L12.5021 9.07391C12.5749 9.15 12.632 9.23971 12.6701 9.33791C12.7124 9.43367 12.7342 9.53722 12.7342 9.64191C12.7342 9.74661 12.7124 9.85015 12.6701 9.94591Z"
                  fill="currentColor"></path>
              </svg>
            </button>
          </div>
        </div>
      </div>
      <div class="p-2 rounded-lg mt-2" style="background: #1f2536">
        <div>
          <div class="flex items-center gap-2">
            <svg class="svg-icon qt-demo" xmlns="http://www.w3.org/2000/svg" width="24" height="24"
              viewBox="0 0 24 24" fill="none">
              <path fill="currentColor" fill-rule="evenodd"
                d="M12.034 3.115a1 1 0 0 1 .932 0l10.5 5.526a1 1 0 0 1 0 1.77l-3.887 2.046v5.47a1 1 0 0 1-.18.572l-.006.009.006-.008h-.001l-.002.003-.003.005-.01.012a2.324 2.324 0 0 1-.115.15 5.772 5.772 0 0 1-.324.365c-.282.292-.7.672-1.263 1.05-1.135.758-2.846 1.494-5.186 1.494s-4.05-.736-5.182-1.495a7.671 7.671 0 0 1-1.26-1.05 5.74 5.74 0 0 1-.438-.516l-.01-.012-.003-.006-.001-.002H5.6s.006.008.002.002l-.003-.003a1 1 0 0 1-.178-.57v-5.47L3 11.183v2.764a1 1 0 1 1-2 0v-4.42a1 1 0 0 1 .534-.886l10.5-5.526ZM4.988 9.084a.5.5 0 0 0 0 .885l7.28 3.831a.5.5 0 0 0 .465 0l7.28-3.831a.5.5 0 0 0 0-.885l-7.28-3.831a.5.5 0 0 0-.466 0l-7.28 3.83ZM7.42 13.51v4.059l.075.08c.197.204.504.487.93.773.847.567 2.174 1.157 4.07 1.157 1.894 0 3.224-.59 4.073-1.157a5.716 5.716 0 0 0 1.01-.855V13.51l-4.613 2.428a1 1 0 0 1-.932 0L7.421 13.51Z"
                clip-rule="evenodd"></path>
            </svg>
            <div>
              <p>QT Demo</p>
              <p style="color: #8ea5c0" class="text-sm">$50,000</p>
            </div>
          </div>
        </div>
      </div>
      <div class="relative mt-2">
        <select style="background: #1f2536"
          class="w-full rounded-md p-2 focus:ring focus:ring-green-500 text-white">
          <option>Forex</option>
          <option>Forex MT5 Real</option>
          <option>Forex MT5 Demo</option>
          <option>Forex MT4 Real</option>
          <option>Forex MT4 Demo</option>
          <option>Shares Real</option>
          <option>Shares Demo</option>
        </select>
      </div>
      <div class="p-2 rounded-lg mt-2" style="background: #1f2536">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-2">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path
                d="M19 0H5C4.20435 0 3.44129 0.316071 2.87868 0.87868C2.31607 1.44129 2 2.20435 2 3V17C2 17.7956 2.31607 18.5587 2.87868 19.1213C3.44129 19.6839 4.20435 20 5 20H19C19.7956 20 20.5587 19.6839 21.1213 19.1213C21.6839 18.5587 22 17.7956 22 17V3C22 2.20435 21.6839 1.44129 21.1213 0.87868C20.5587 0.316071 19.7956 0 19 0ZM20 17C20 17.2652 19.8946 17.5196 19.7071 17.7071C19.5196 17.8946 19.2652 18 19 18H5C4.73478 18 4.48043 17.8946 4.29289 17.7071C4.10536 17.5196 4 17.2652 4 17V3C4 2.73478 4.10536 2.48043 4.29289 2.29289C4.48043 2.10536 4.73478 2 5 2H19C19.2652 2 19.5196 2.10536 19.7071 2.29289C19.8946 2.48043 20 2.73478 20 3V17ZM12 6C11.4696 6 10.9609 6.21071 10.5858 6.58579C10.2107 6.96086 10 7.46957 10 8C10.0021 8.34903 10.0955 8.69143 10.2709 8.99317C10.4464 9.29491 10.6977 9.54549 11 9.72V13C11 13.2652 11.1054 13.5196 11.2929 13.7071C11.4804 13.8946 11.7348 14 12 14C12.2652 14 12.5196 13.8946 12.7071 13.7071C12.8946 13.5196 13 13.2652 13 13V9.72C13.3023 9.54549 13.5536 9.29491 13.7291 8.99317C13.9045 8.69143 13.9979 8.34903 14 8C14 7.46957 13.7893 6.96086 13.4142 6.58579C13.0391 6.21071 12.5304 6 12 6Z"
                fill="currentColor"></path>
              <path d="M5 20H10V21C10 21.5523 9.55228 22 9 22H6C5.44772 22 5 21.5523 5 21V20Z" fill="currentColor">
              </path>
              <path d="M14 20H19V21C19 21.5523 18.5523 22 18 22H15C14.4477 22 14 21.5523 14 21V20Z"
                fill="currentColor"></path>
            </svg>
            <p class="text-white">My Safe</p>
          </div>
          <p class="px-3 py-1 rounded-md" style="
              border: 1px solid #047838;
              background-color: #074f40;
              font-size: 10px;
            ">
            Open
          </p>
        </div>
      </div>
      <button class="w-full p-2 rounded-lg mt-2 text-xs" style="
          background: #1d2130;
          border: 1px solid #343e52;
          color: #8ea5c0;
        ">
        Profile >>
      </button>
    </div>
  </div>
<script>
  document.addEventListener("DOMContentLoaded", () => {
    const walletButton = document.getElementById("walletButton");
    const walletModal = document.getElementById("walletModal");
    const closeWalletModal = document.getElementById("closeWalletModal");

    function closeModal() {
      walletModal.classList.add("hidden");
    }

    if (walletButton) {
      walletButton.addEventListener("click", (e) => {
        e.stopPropagation();
        walletModal.classList.remove("hidden");
      });
    }

    if (closeWalletModal) {
      closeWalletModal.addEventListener("click", closeModal);
    }

    // Close modal when clicking outside the modal content
    walletModal.addEventListener("click", (e) => {
      // If click is directly on the modal container (backdrop area)
      if (e.target === walletModal) {
        closeModal();
      }
    });

    // Close on escape key
    document.addEventListener("keydown", (e) => {
      if (e.key === "Escape" && !walletModal.classList.contains("hidden")) {
        closeModal();
      }
    });
  });
</script>