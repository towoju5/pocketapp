<div class="h-16 border-b border-[#2a3350] flex items-center justify-between gap-3.5 px-5 flex-shrink-0 relative box-border w-full">
    ...
    <div class="relative">
        <button type="button" id="balanceMenuBtn" class="bg-transparent border-0 cursor-pointer text-right px-2 py-1 rounded-lg flex items-center gap-2 text-[#d7dcea]">
            <span>
                <span class="block text-[11px] text-[#7c86a3]">{{ ucfirst($wallet_balance['name'] ?? 'Wallet') }}</span>
                <span class="block text-[15px] font-semibold">{{ formatPrice($wallet_balance['balance'] ?? 0) }}</span>
            </span>
    ...
    <a href="{{ route('wallet.index') }}" class="bg-[#16c087] text-white rounded-[9px] px-[18px] py-[11px] font-bold text-[13px] flex items-center gap-1.5" style="text-decoration:none;">
